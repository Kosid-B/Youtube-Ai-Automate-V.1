...
ยิงจริงทีละบริการแล้วบอกว่าอันไหนใช้ไม่ได้ พร้อมวิธีแก้ — ตรวจครบทุกข้อ
แล้วค่อยสรุปทีเดียว ไม่หยุดที่ข้อแรกที่พัง (ไฟล์ตั้งค่าที่ผิดมักผิดหลายจุดพร้อมกัน)

จับได้ถึงกรณีที่มองด้วยตาไม่เห็น เช่น เอา anon key ไปใส่ช่อง service_role
(หน้าตาเป็น JWT เหมือนกัน แต่ worker จะโดน RLS ปฏิเสธทุก query),
ตั้งชื่อเสียงพากย์ที่ไม่มีอยู่จริงในบัญชี, และ **ตั้งชื่อฟอนต์ที่เครื่องไม่มี**
ซึ่งเป็นโหมดพังที่เงียบที่สุดของโปรเจค

> ⚠️ ห้ามตั้งชื่อสคริปต์นี้ว่า `doctor` — `pnpm doctor` เป็นคำสั่งในตัวของ pnpm
> ซึ่งจะถูกเรียกแทนโดยไม่มีอะไรเตือน และผลที่ขึ้นก็ดูเหมือนผ่านหมดเสียด้วย
